...
async function insertLanguageInDB(package_name, languagePercentages, loc) {
    ...
    try {
        ...
        // package_nameに一致するデータを検索
        const filter = { Name: package_name };
        ...
        if (updateResult.matchedCount === 0) {
            console.log(`No document found with package: ${package_name}`);
        } else {
            console.log(`Successfully updated document with package: ${package_name}`);
        }
    } catch (err) {
...
async function main() {
    ...
    try {
        ...
        await insertLanguageInDB(package_name, result, loc);
    } catch (error) {
